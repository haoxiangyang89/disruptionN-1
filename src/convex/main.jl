...
function solveMain(T, Δt, N, form_mode, allGen = false, iterMin = 100,
    ...
    while (keepIter)&(iterNo <= iterMax)
        ...
        if simuRule == 0
            if simuSample == Dict()
                trialPaths,currentLB,currentUBDict = exeForward(T, Δt, N, form_mode, Dict(), hardened);
            else
                pathIter = Dict();
                for sNo in 1:N
                    pathIter[sNo] = simuSample[sampleCounter+sNo];
                end
                trialPaths,currentLB,currentUBDict = exeForward(T, Δt, N, form_mode, pathIter, hardened);
                sampleCounter += N;
            end
        else
            trialPaths,currentLB,currentUBDict = exeForward_simuOpt(T, Δt, N, iterNo, hardened);
        end
        ...
        if !(ubGen)
            ...
            trialPathsUB,currentLBUB,currentUBDict = exeForward(T, Δt, ubM, form_mode, Dict(), hardened);
            currentUBList = [currentUBDict[ubkey] for ubkey in keys(currentUBDict)];
        end
        ...
        if (currentLB <= currentUBu)&(currentLB >= currentUBl)&(iterNo >= iterMin)
            ...
            if allGen
                exeBackwardAll(T, Δt, trialPaths, form_mode, hardened);
            else
                exeBackward(T, Δt, trialPaths, form_mode, hardened);
            end
        end
...
function preGen(T, Δt, N, iterMax, form_mode, cutDict = Dict(), hardened = [])
    # pregenerate cuts
    ...
    while iterNo <= iterMax
        iterNo += 1;
        trialPaths,currentLB,currentUBDict = exeForward_last(T, Δt, N, form_mode, hardened);
        exeBackward_last(T, Δt, trialPaths, form_mode, hardened);
    end
